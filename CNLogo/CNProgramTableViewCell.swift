...
class CNProgramTableViewCell: UITableViewCell {
    ...
    func setup(item: CNProgramTableViewItem, height: CGFloat) throws {
        var x: CGFloat = 5.0
        
        let bubbles = try item.block.createBubbles(item.block, height: height, prefix: item.startIndex == nil ? "" : "End of ")
